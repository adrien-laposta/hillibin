...
class Hillipop(_InstallableLikelihood):
    ...
    def _select_spectra(self, cl, mode=0):
        ...
        for xf in range(self.nxfreq):
            ...
            xl_temp = acl[xf, :]
            if self.bin_data == True:

                ell_temp = np.arange(len(xl_temp))
                id_bin = np.where(binning < np.max(ell_temp))
                binning = binning[id_bin]
                ell_temp, xl_temp = bin_spectra(ell_temp, xl_temp, binning)
                id = np.where((ell_temp >= lmin) & (ell_temp <= lmax))
                ell_temp, xl_temp = ell_temp[id], xl_temp[id]
                xl_temp = list(xl_temp)
                ell_temp = list(ell_temp)
            else:

                xl_temp = list(xl_temp[lmin: lmax + 1])
                ell_temp = list(np.arange(lmin,lmax+1))
